...
- En el objeto `state`, agregar la variable `activeNumber` inicializada en una cadena vacía `''`.
- En la función `updateUI()`:
  - Cachear el elemento `$callInfo = $('#webphone-call-info')`.
  - En el caso `idle` del switch de estados de llamada, limpiar el número activo (`state.activeNumber = '';`).
  ...
  - Si es falso, ocultar el elemento `$callInfo` y limpiar su contenido.
...
- **Limpieza de Estado:** Asegurar que `state.activeNumber` se limpie completamente al regresar al estado `idle` para evitar que el número de una llamada previa permanezca en pantalla.
